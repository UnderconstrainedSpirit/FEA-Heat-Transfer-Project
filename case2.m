...
h = 10;     % in W / m^2-K
T0 = 298;   % ambient temperature in K (for convection)
...
for i = 1:ne
    ...
    if i >=1 && i < 26                              % Convective heat transfer
        n_sol = solve(y == 0, n);
        mf = diff(subs(x, n, n_sol), e);
        new_N = subs(N, n, n_sol);
        I2 = new_N' * new_N * h * mf;
        I3 = new_N' * new_N * ones([101 1]) * h * T0 * mf;
        E3 = int(I2, e, 0, 1);
        E4 = int(I3, e, 0, 1);
        F1 = F1 + E4;
        K1 = K1 + E3;
